...
export default function ConsumerActionSection() {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      ScrollTrigger.getById("consumer-section-pin")?.kill() // IDをユニークに
      ...
      if (scrollHeight > 0) {
        ...
        ScrollTrigger.create({
          ...
          end: `+=${scrollHeight}`, // rightSidebarのスクロール可能な高さ分だけ固定を継続 + 50pxのオフセット
          pin: true, // セクションを固定
    ...
    if (initialScrollHeight > 0) {
      ScrollTrigger.create({
        ...
        trigger: section,
        start: "top top",
        end: `+=${initialScrollHeight}`, // + 50pxのオフセット
        pin: true,
        scrub: "power3.inOut", // 修正: easeInOutCubicに相当するGSAPイージングを適用
        snap: {
          snapTo: 1,
    ...
    return () => {
      ScrollTrigger.getById("consumer-section-pin")?.kill() // このセクションのScrollTriggerをキル
